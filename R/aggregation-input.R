...
tlfStatFunctions <- enum(c(
  ...
  sapply(predefinedPercentiles, function(percentileValue){paste0("Percentile-", percentileValue, "%")}),
...
for (percentileValue in predefinedPercentiles){
  assign(paste0("Percentile-", percentileValue, "%"), function(x){stats::quantile(x, probs = percentileValue/100)})
}

assign("mean-1.96sd", function(x){return(mean(x)-1.96*stats::sd(x))})
assign("mean+1.96sd", function(x){return(mean(x)+1.96*stats::sd(x))})

...
patchwork::wrap_plots(emptyPlot, myEmptyPlot, ncol = 2)
```
...
patchwork::wrap_plots(pLab1, pLab2, ncol = 2)
```
...
Additionally, a few predifined themes are available :

```{r list of json themes}
list.files(system.file("themes", package = "tlf"))
```

These predifined themes can be directly loaded and used through functions named `use<ThemeName>Theme()`.

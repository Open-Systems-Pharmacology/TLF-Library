...
load("timeProfileDataFrame.RData")
library(tlf)
```
...
```{r}
minimalGrouping <- tlf:::Groupings$new( linetype = "IndividualID" )
```
...
grouping3 <- GroupMapping$new(group = mappingDataFrame)
```
...
```{r, results='asis'}
grouping4 <- GroupMapping$new(group = binningDataFrame)
```

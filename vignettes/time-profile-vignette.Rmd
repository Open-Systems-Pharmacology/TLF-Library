...
```{r, results='asis'}
pander::pandoc.table(aggSummary$dfHelper$SimulatedMin, round=2)
```

```{r, results='asis'}
pander::pandoc.table(aggSummary$dfHelper$SimulatedMean, round=2)
```

The `metaData` corresponding to the columns of the resulting dataframes are lists that are stored together in a list named `aggSummary$metaDataHelper`.  For this example, the two `metaData` lists in `aggSummary$metaDataHelper` are  `aggSummary$metaDataHelper$SimulatedMin` and `aggSummary$dfHelper$SimulatedMean`.  The contents of the list `aggSummary$metaDataHelper$SimulatedMean` are:
...
The following section discusses how `Groupings` objects can be used in 

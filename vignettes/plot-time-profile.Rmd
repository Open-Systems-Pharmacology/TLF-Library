...
- **`observedDataMapping`**: An `ObservedDataMapping` object mapping `x`, `y`, `ymin`, `ymax`, `lloq` and aesthetic groups to their variable names of `observedData`.
...
The object also includes the field `lloq` for variables defining the lower limit of quantification of the data. Such data are usually censored and handled differently during modeling.
...
## 3.2.5. Lower limit of quantification (lloq)
...
```{r combine simulated and observed}
